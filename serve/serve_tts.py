...
image = (
    ...
    .pip_install("fastapi", "uvicorn[standard]", "pydantic", "soundfile", "numpy", "torchcodec")
    .run_commands(
...
@app.cls(
    ...
    min_containers=1,  # Keeps TTS container alive 24/7 for zero cold start delays!
)
class TTSService:
    @modal.enter()
    def load_model(self):
        ...
        # Monkeypatch torch.load to default weights_only=False globally to bypass safe globals unpickling failures in PyTorch 2.6
        ...
        print("[TTS] Monkeypatched torch.load to bypass weights_only restrictions.")
        ...
        if use_fine_tuned:
            ...
            # Strip "xtts." prefix from fine-tuned state dict keys to align correctly with base model keys
            clean_state_dict = {}
            ...
            print(f"[TTS] Matched and filtered state dict keys: {len(filtered_state_dict)} / {len(clean_state_dict)}")
